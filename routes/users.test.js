...
describe("PATCH /users/:username", () => {
  ...
  test("fails: set new password for other user", async function () {
    const resp = await request(app)
        .patch(`/users/u1`)
        .send({
          password: "new-password",
...
describe("POST /users/:username/jobs/:id", () => {
  test("works for current user", async () => {
    ...
    const resp = await request(app)
        .post(`/users/u1/jobs/${jobs[1].id}`)
        .send({
          apply: true
        })
        .set("authorization", `Bearer ${u1Token}`);
    expect(resp.body).toEqual({ applied: `${jobs[1].id}` });
    expect(resp.statusCode).toEqual(201);
  })
  ...
  test("works for admins", async () => {
    const jobs = await Job.findAll()
    const resp = await request(app)
        .post(`/users/u1/jobs/${jobs[2].id}`)
        .send({
          apply: true
        })
        .set("authorization", `Bearer ${u2Token}`);
    expect(resp.body).toEqual({ applied: `${jobs[2].id}` });
    expect(resp.statusCode).toEqual(201);
  })

  test("fails for wrong user", async () => {
    const jobs = await Job.findAll()
    const resp = await request(app)
        .post(`/users/u3/jobs/${jobs[0].id}`)
        .send({
          apply: true
        })
        .set("authorization", `Bearer ${u1Token}`);
  ...
  test("fails for duplicate submissions", async () => {
    const jobs = await Job.findAll()
    const resp = await request(app)
        .post(`/users/u3/jobs/${jobs[2].id}`)
        .send({
          apply: true
        })
        .set("authorization", `Bearer ${u3Token}`);
